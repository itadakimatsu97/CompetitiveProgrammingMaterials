...
int numberOfDigits(int k){
    return static_cast<int>(log10(static_cast<double>(k)) + 0e-9) + 1;
}
...
int main() {
    ...
    int digitCount = 1;
    for (int k = 2; ; ++k){
        digitCount += numberOfDigits(k);
        int tmp = S.back() + digitCount;
        if (tmp > 2147483647 || tmp < 0) //tmp<0 to check out of limit of INT
            break;
        S.push_back(tmp);
    }
    ...
    while (T--) {
        int pos;
        cin >> pos;
        int index = lower_bound(all(S), pos) - S.begin() - 1;
        //Result is serieK = serieIndex at pos - S[index]
        // cout << index << "  " << pos - S[index];
        string str = "0";
        For(i, 1, index + 1) {
            str += to_string(i);
        }
        cout << str[pos - S[index]] << '\n';
    }
    return 0;
}