...
int main() {
    ...
    vector<int> index[257]; //List A-z ascii 256 chars
    string abc;
    cin >> abc;
    int id = 0;
    while (id < abc.length()) {
    ...
    while(T--){
        int firstId = 0;
        int lastId =-1; //0 always > -1;
        bool found = true;
        string query;
        cin >> query;
        Rep(i, query.length()){
            auto pos = upper_bound(all(index[query[i]]), lastId);
            if(pos == index[query[i]].end()){
                found = false;
                break;
            }
            lastId = *pos;
